 ...
 * */
/*
maxFileSize: 개별 파일의 최대용량을 lMB로 설정
...
public class UploadProcess extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		try {
			...
			String saveDirecotory = getServletContext()
					.getRealPath("/Uploads");
			//파일 업로드 처리
			String originalFileName = FileUtil
					.uploadFile(req, saveDirecotory);
			//서버에 저장된 파일명 변경
			String savedFileName = FileUtil
					.renameFile(saveDirecotory, originalFileName);
			//파일목록 출력을 위한 페이지로 이동
			resp.sendRedirect("FileList.jsp");
		}
		catch(Exception e) {
			e.printStackTrace();
			//만역 업로드에 실패하면 request영역에 메세지를 저장한 후
