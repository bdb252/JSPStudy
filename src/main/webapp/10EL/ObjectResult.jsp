...
<body>
	...
	EL 내장객체 없이 속성명만으로 접근할 수 있다. 
	즉 requestScope.personObj.name 형태로 기술하지 않아도 된다. -->
	<h2>영역을 통해 전달된 객체 읽기</h2>
	<!-- 
	"속성명.멤버변수" 형태로 기술하면 클래스에 선언된 getter를 자동으로 호출하여 값을 출력한다.
	즉 아래 문장은 getter를 호출하므로 Person클래스에서 getName()을 주석처리하면
	에러가 발생한다. 
	 -->
	<ul>
		<!-- getter/setter를 사용하지 않아도 private의 멤버변수를 바로 접근해도 
		 내부적으로는 그 멤버변수에 해당하는 getter를 호출하기때문에 아래와 같이 사용 가능하다. -->
	...
	//출력시에는 멤버변수에 접근을 위해 getter를 호출해야 한다.
	...
	따라서 산술연산을 하려면 숫자형식으로 변환해야 한다. 하지만 EL에서는 
