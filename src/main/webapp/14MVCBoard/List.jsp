...
<c:choose>
	...
	<!-- boardLists에 저장된 값이 없다면 등록된 게시물이 없거나, 검색된 게시물이 없는 
	경우이므로 아래와 같이 출력한다. -->
        <tr>
	...
	<c:otherwise>
        ...
        <tr align="center">
            <td>
            	<!-- totalCount(게시물 개수)에서 index(0부터 시작)를 
            	순차적으로 차감한다. 게시물의 번호를 출력 -->
            	${ map.totalCount - loop.index }
            </td>
            <td align="left">
            	<!-- 제목 클릭 시 '열람'페이지로 이동해야 하므로 게시물의
            	일련번호를 파라미터로 전달한다. -->
            	<a href="../mvcboard/view.do?idx=${ row.idx }">
            		${ row.title }</a>
            </td> 
            <!-- 현재 루프에서 row는 MVCBoadrDTO를 의미하므로 각 멤버변수의
            getter를 통해 저장된 값을 출력한다. -->
            <td>${ row.id }</td>
