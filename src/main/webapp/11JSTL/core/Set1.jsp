...
<body>
	<!-- 
	set태그
	: 변수 선언할떄 사용. JSP의 setAttribute()메서드와 동일한 기능으로
	  4가지 영역에 속성을 저장한다.
	  var: 속성명(변수명)을 지정.
	  value: 속성에 저장할 값
	  scope: 4가지 영역명을 지정. 생략시 page 영역이 지정됨.
	  target: set태그를 통해 생성된 자바빈의 이름을 지정함
	  property: target으로 지정한 자바빈의 멤버변수값을 지정
	-->
	
	<!-- 
	변수 선언 : 여기서는 가장 좁은 page 영역에 저장된다.
	...
	<!-- 
	인스턴스를 먼저 생성한 후 target, property를 통해 멤버변수의 값을 초기화한다.
	이때는 자바빈의 setter 메서드가 사용된다.	 -->
